#!/bin/bash
# Klingon HTLC Refund Test Script
# Tests the CSV timelock refund path for HTLC swaps when counterparty disappears
#
# HTLC Refund Flow:
# 1. Setup HTLC swap between Node 1 (Maker/BTC) and Node 2 (Taker/LTC)
# 2. Both sides fund the P2WSH escrow
# 3. Node 2 "disappears" (simulated by not revealing/claiming)
# 4. Wait for CSV timelock to expire
# 5. Node 1 refunds their BTC via script path (OP_ELSE branch)
#
...
show_balances() {
    ...
    echo "$btc_scan" > /tmp/htlc_refund_node1_btc_scan.json
    echo "$ltc_scan" > /tmp/htlc_refund_node2_ltc_scan.json

    echo "$btc_scan" | jq -r '.result.addresses | sort_by(.balance) | reverse | .[0] // empty' > /tmp/htlc_refund_node1_btc_funded.json
    echo "$ltc_scan" | jq -r '.result.addresses | sort_by(.balance) | reverse | .[0] // empty' > /tmp/htlc_refund_node2_ltc_funded.json
}
...
create_order() {
    ...
    echo "$ORDER_ID" > /tmp/htlc_refund_order_id
    sleep 2
}
...
take_order() {
    ...
    ORDER_ID=$(cat /tmp/htlc_refund_order_id)
    ...
    echo "$TRADE_ID" > /tmp/htlc_refund_trade_id
    sleep 2
}

init_swaps() {
    log "Initializing HTLC swaps on both nodes..."
    TRADE_ID=$(cat /tmp/htlc_refund_trade_id)
...
get_escrow_addresses() {
    ...
    echo "$btc_escrow" > /tmp/htlc_refund_btc_escrow
    echo "$ltc_escrow" > /tmp/htlc_refund_ltc_escrow

    info "BTC Escrow (P2WSH): $btc_escrow"
    info "LTC Escrow (P2WSH): $ltc_escrow"
}
...
fund_escrows() {
    ...
    if [ -n "$btc_txid" ]; then
        ...
        echo "$btc_txid" > /tmp/htlc_refund_btc_funding_txid
    ...
    if [ -n "$ltc_txid" ]; then
        ...
        echo "$ltc_txid" > /tmp/htlc_refund_ltc_funding_txid
...
set_funding_info() {
    ...
    local btc_txid=$(cat /tmp/htlc_refund_btc_funding_txid 2>/dev/null)
    local ltc_txid=$(cat /tmp/htlc_refund_ltc_funding_txid 2>/dev/null)
...
check_timeouts() {
    ...
    curl -s $NODE1_API -d '{"jsonrpc":"2.0","method":"swap_checkTimeouts","id":1}' | jq '.result'

    echo ""
    info "=== Node 2 Timeout Check ==="
    curl -s $NODE2_API -d '{"jsonrpc":"2.0","method":"swap_checkTimeouts","id":1}' | jq '.result'
}

attempt_refund() {
    log "Attempting HTLC refund on Node 1 (BTC)..."
    TRADE_ID=$(cat /tmp/htlc_refund_trade_id)

    local result=$(curl -s $NODE1_API -d "{
        \"jsonrpc\":\"2.0\",
        \"method\":\"swap_htlcRefund\",
    ...
    if [ -n "$txid" ]; then
        highlight "HTLC REFUND SUCCESSFUL!"
        log "Refund txid: $txid"
        echo ""
        log "View on explorer: https://mempool.space/testnet/tx/$txid"
    else
        local err=$(echo "$result" | jq -r '.error.message // "unknown error"')
...
attempt_refund_ltc() {
    log "Attempting HTLC refund on Node 2 (LTC)..."
    TRADE_ID=$(cat /tmp/htlc_refund_trade_id)
...
show_refund_instructions() {
    echo ""
    echo "========================================"
    echo " HTLC REFUND TEST - SWAP ABANDONED"
    echo "========================================"
    echo ""

    TRADE_ID=$(cat /tmp/htlc_refund_trade_id 2>/dev/null || echo "N/A")
    BTC_ESCROW=$(cat /tmp/htlc_refund_btc_escrow 2>/dev/null || echo "N/A")
    LTC_ESCROW=$(cat /tmp/htlc_refund_ltc_escrow 2>/dev/null || echo "N/A")
    BTC_TXID=$(cat /tmp/htlc_refund_btc_funding_txid 2>/dev/null || echo "N/A")
    LTC_TXID=$(cat /tmp/htlc_refund_ltc_funding_txid 2>/dev/null || echo "N/A")
    ...
    echo ""
    echo "HTLC Refund uses the OP_ELSE branch of the script:"
    echo "  <timeout_blocks> OP_CHECKSEQUENCEVERIFY OP_DROP"
    echo "  <sender_pubkey> OP_CHECKSIG"
    echo ""
    ...
    echo "   ./scripts/test-htlc-refund.sh timeout"
    echo ""
    echo "2. Attempt refund (will fail until timeout):"
    echo "   ./scripts/test-htlc-refund.sh refund-btc    # Node 1 refunds BTC"
    echo "   ./scripts/test-htlc-refund.sh refund-ltc    # Node 2 refunds LTC"
    echo ""
    echo "3. Check swap status:"
    echo "   ./scripts/test-htlc-refund.sh status"
    echo ""
    echo "4. Check balances after refund:"
    echo "   ./scripts/test-htlc-refund.sh balances"
    echo ""
...
case "${1:-setup}" in
    ...
    setup)
        echo ""
        echo "========================================"
        echo " HTLC REFUND TEST SETUP"
        echo "========================================"
        echo ""
        echo "This script sets up an HTLC swap and then ABANDONS it to test the refund path."
        ...
        echo ""
        echo "Unlike MuSig2 (Taproot), HTLC refunds use the script's OP_ELSE branch"
        echo "which requires OP_CHECKSEQUENCEVERIFY to pass."
        echo ""
    ...
    *)
        ...
        echo "  setup       - Full setup: start nodes, create HTLC swap, fund, then ABANDON (default)"
        ...
        echo ""
        echo "Typical flow:"
        echo "  1. ./scripts/test-htlc-refund.sh setup    # Setup and fund, then abandon"
        echo "  2. ./scripts/test-htlc-refund.sh timeout  # Check how long until refund available"
        echo "  3. [wait for timeout to expire]"
        echo "  4. ./scripts/test-htlc-refund.sh refund   # Get funds back via OP_ELSE branch"
        echo "  5. ./scripts/test-htlc-refund.sh balances # Verify refund succeeded"
        exit 1
        ;;
esac
